...
- **Source routing**: `/api/run` takes `listing_source`/`parcel_source` (default `auto`), resolved per market via `MARKET_SOURCES` in `main.py` (miami-dade-fl→miamidade, paradise-valley-az→maricopa, port-royal-naples-fl→collier, bradenton-anna-maria-fl→manatee, hamptons-ny→nys, charlotte-nc→mecklenburg, las-vegas-henderson-nv→clark). Explicit names (`fixture`, etc.) still work.
- **Not yet scannable** (no public county feed found with the required owner/year-built/value attributes): bel-air-beverly-hills-ca (LA County publishes roll values + year built at `Parcel_Data_2021_Table` but no owner names), seattle-eastside-wa (King County parcels have values but no year built/owner), atherton-ca (San Mateo County/Atherton publish values but no owner/year built). The licensed `attom` provider is the intended fallback once `ATTOM_API_KEY` is provisioned.
